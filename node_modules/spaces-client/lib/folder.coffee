...
class Folder
  ...
  @showFolder = (site, sessionId, resourceId, parentId, userId, onsuccess, onfail) ->
    addlHeaders = {
      "Accept": "application/vnd.moxiesoft.spaces-v1+json",
      "Cookie": ["_social_navigator_session=" + sessionId, "path=/"]
    }
    Resource.getResource(site, userId, sessionId, 'document_libraries/' + resourceId, addlHeaders, onsuccess, onfail)

  @createFolder = (site, sessionId, resourceId, parentId, userId, onsuccess, onfail, oncreate) ->
    data = {
      document_library: {
...
#        tags: Faker.Lorem.words(3).join(" "),
    ...
    Resource.createResource(site, userId, sessionId, 'document_libraries', data, onsuccess, onfail, oncreate)

  @updateFolder = (site, sessionId, resourceId, parentId, userId, onsuccess, onfail) ->
    data = {
      document_library: {
        title: Faker.Lorem.sentences(1)
      }
    }
    Resource.editResource(site, userId, sessionId, 'document_libraries/' + resourceId, data, onsuccess, onfail)

  @deleteFolder = (site, sessionId, resourceId, parentId, userId, onsuccess, onfail, ondestroy) ->
    Resource.deleteResource(site, userId, sessionId, 'document_libraries/' + resourceId, onsuccess, onfail, ondestroy)

  @showDocument = (site, sessionId, resourceId, parentId, userId, onsuccess, onfail) ->
    addlHeaders = {
      "Accept": "application/vnd.moxiesoft.spaces-v1+json",
      "Cookie": ["_social_navigator_session=" + sessionId, "path=/"]
    }
    Resource.getResource(site, userId, sessionId, 'documents/' + resourceId, addlHeaders, onsuccess, onfail)

  @createDocument = (site, sessionId, resourceId, folderId, userId, onsuccess, onfail, oncreate) ->
    ...
    fs.readdir("./assets/files/", ( (err, files) ->
      ...
      fs.stat("./assets/files/" + fileName, ((err, stats) ->
        throw err if err
        fileSize = stats.size
        Folder.initS3PresignedPost(site, userId, sessionId, ((data) ->
          ...
          Folder.uploadToS3(site, userId, data, fileName, fileSize, ( ->
            ...
            Folder.createDocumentMetadata(folderId, site, userId, sessionId, data, fileName, fileSize, ( (response) ->
              documentId = response.match(/"id":"(.*?)"/)[1]
              Spaces.logger.debug("[%s][%s] Created document [%s]", site.site_id, userId, documentId)
              oncreate(documentId) if oncreate
              onsuccess()
            ), ((message) ->
  ...
  @updateDocument = (site, sessionId, resourceId, parentId, userId, onsuccess, onfail) ->
    data = {
      document: {
        title: Faker.Lorem.sentences(1)
      }
    }
    Resource.editResource(site, userId, sessionId, 'documents/' + resourceId, data, onsuccess, onfail)

  @deleteDocument = (site, sessionId, resourceId, parentId, userId, onsuccess, onfail, ondestroy) ->
    Resource.deleteResource(site, userId, sessionId, 'documents/' + resourceId, onsuccess, onfail, ondestroy)

  @getPersonalDocLibId = (site, userId, sessionId, onsuccess) ->
    Spaces = require('./spaces-client')
    headers = {
      "Accept": "application/vnd.moxiesoft.spaces-v1+json",
      "Cookie": ["_social_navigator_session=" + sessionId, "path=/"]
    }
    Resource.getResource(site, userId, sessionId, 'current_user/document_library.json', headers, ((msecs, folder) ->
      folderId = folder.match(/"id":"(.*?)"/)[1]
      onsuccess(folderId)
    ), (() ->
      Spaces.logger.info("[%s][%s] Folder.getPersonalDocLibId Error", site.site_id, userId)
    ))

  @initS3PresignedPost = (site, userId, sessionId, onsuccess, onfail) ->
    Spaces = require('./spaces-client')
...
#        Spaces.logger.debug("[%s][%s] Folder.uploadToS3 request ended: %s - %s", site.site_id, userId, res.statusCode, res.status)
  ...
  @createDocumentMetadata = (folderId, site, userId, sessionId, data, fileName, fileSize, onsuccess, onfail) ->
    Spaces = require('./spaces-client')
