...
(function() {
  ...
  Resource = (function() {
    ...
    Resource.hrdiff = function(t1, t2) {
      var mms, s;
      s = t2[0] - t1[0];
      mms = t2[1] - t1[1];
      return s * 1e9 + mms;
    ...
    Resource.getResource = function(site, userId, sessionId, sPath, addlHeaders, onsuccess, onfail) {
      ...
      req = require('https').request(opts, (function(res) {
        ...
        return res.on('end', (function() {
          var msecs;
          msecs = Resource.hrdiff(t1, process.hrtime());
          if (res.statusCode === 200) {
            return onsuccess(msecs, resp);
          } else {
            Spaces.logger.error("[%s][%s] FAIL: GET %s (%d - %s)\n%s", siteId, userId, sPath, res.statusCode, res.status, resp);
            return onfail(msecs);
          }
        }));
      }));
