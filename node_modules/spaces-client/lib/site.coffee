# curl -X POST -H "CONTENT-TYPE:application/json" -H "ACCEPT:application/vnd.moxiesoft.spaces-v1+json" -H "API-KEY:3ab3cdd7ed3c591688fe5bd2714fe043" -d 'blah' https://perf-mothership.moxiespaces.com/api/sites

class Site
  @createSite: (data, onsuccess, onfail) ->
    Spaces = require('./spaces-client')
    $ = require("jquery")
    $.ajax Spaces.mothershipUrl + '/api/sites',
      type: 'POST',
      dataType: 'json',
      ...
      beforeSend: (request) ->
        ...
        request.setRequestHeader("API-KEY", Spaces.apiKey)
      error: (jqXHR, textStatus, errorThrown) ->
        Spaces.logger.error("Error creating site: %s - %s - %s", textStatus, errorThrown, jqXHR)
        onfail(errorThrown || textStatus)
      success: (data) ->
        siteId = JSON.stringify(data).match(/"site_id":"(.*?)"/)[1]
        Spaces.logger.info('[%s][%s] Created Site', siteId, 'Admin')
        onsuccess(data)

  # why are we using the https module instead of jquery? Because jquery denies us access to http-only cookies.
  ...
  @tryUntilSuccess: (data, options, onsuccess, onerror) ->
    ...
    req.on('error', ((e) ->
      Spaces.logger.error('Unable to log in: %s', e.message)
      onerror(e.message)
    ))
